...
async function loadCombinedUserData(targetUserId) {
    ...
    if (loggedInUser && !isOwnProfile) {
        ...
        } catch (viewerError) { viewerProfileData = { id: loggedInUser.uid, friends: {} }; }
    }
    ...
    try {
        ...
        let profileSnap = await userProfileRef.get(); let profileData = null;
        if (!profileSnap || !profileSnap.exists) {
            ...
            if (profileData.bannerUrl === undefined) profileData.bannerUrl = null;
        ...
        if (profileData && competitiveStatsData) {
            if (areStatsDifferent(competitiveStatsData, profileData.leaderboardStats)) {
                try { const statsToSave = { ...competitiveStatsData }; delete statsToSave.id;
                    await userProfileRef.update({ leaderboardStats: statsToSave });
                    profileData.leaderboardStats = statsToSave;
                } catch (updateError) { console.error(`Error syncing stats:`, updateError); }
            }
        }
        ...
        if (isOwnProfile && viewingUserProfileData.stats) {
             ...
             if (definitions) {
                ...
                if (potentiallyUpdatedProfile) {
                    viewingUserProfileData.profile = potentiallyUpdatedProfile; viewerProfileData = viewingUserProfileData.profile;
                    saveCombinedDataToCache(targetUserId, viewingUserProfileData);
                    displayProfileData(viewingUserProfileData.profile, isOwnProfile); displayBanner(viewingUserProfileData.profile.bannerUrl);
                    const latestUnlockedIds = await fetchUserUnlockedAchievements(targetUserId); await displayAchievementsSection(viewingUserProfileData.stats, latestUnlockedIds);
                    displayFriendsSection(viewingUserProfileData.profile);
                }
             } else { console.warn("Skipping achievement grant check: definitions failed load."); }
         }
...
function displayProfileData(profileData, isOwner) {
    ...
    if (profileData.profilePictureUrl) {
        ...
        profileImage.onerror = () => { profileImage.style.display = 'none'; profileInitials.textContent = displayName?.charAt(0)?.toUpperCase() || '?'; profileInitials.style.display = 'flex'; };
    } else {
        profileImage.style.display = 'none'; profileImage.src = ''; profileInitials.textContent = displayName?.charAt(0)?.toUpperCase() || '?'; profileInitials.style.display = 'flex';
    }
...
function displayCompetitiveStats(statsData) {
    ...
    if (!statsData || typeof statsData !== 'object' || Object.keys(statsData).length === 0) {
        competitiveStatsDisplay.innerHTML = '<p class="list-message">Competitive stats unavailable.</p>'; return;
    }
...
function displayPoxelStats(poxelData, message = null) {
    ...
    poxelStatsSectionWrapper.style.display = 'block'; // Ensure wrapper is visible
    ...
    if (!poxelData || typeof poxelData !== 'object' || Object.keys(poxelData).length === 0) {
         poxelStatsDisplay.innerHTML = '<p class="list-message">Poxel.io stats unavailable.</p>'; return;
    }
    ...
    for (const key in statsMap) {
         if (poxelData.hasOwnProperty(key) && poxelData[key] !== null && poxelData[key] !== undefined) {
             ...
             poxelStatsDisplay.appendChild(createStatItem(statsMap[key], value)); statsAdded++;
         }
    }
    if (poxelData.hasOwnProperty('kills') && poxelData.hasOwnProperty('deaths') && poxelData.deaths !== null && poxelData.kills !== null) {
         const kills = Number(poxelData.kills) || 0; const deaths = Number(poxelData.deaths) || 0;
         const kd = deaths > 0 ? (kills / deaths).toFixed(2) : kills.toFixed(2);
         poxelStatsDisplay.appendChild(createStatItem('Poxel K/D', kd)); statsAdded++;
    }
...
async function fetchUserUnlockedAchievements(userId) {
    if (!userId) return [];
    try { const userAchievementsRef = db.collection('userAchievements').doc(userId);
        const doc = await userAchievementsRef.get();
        return doc.exists ? (doc.data()?.unlocked || []) : [];
    } catch (error) { console.error(`Error fetching unlocked achievements for ${userId}:`, error); return []; }
}

function calculateAchievementProgress(achievement, userStats) {
     const criteria = achievement?.criteria || {}; const targetValue = criteria.value || 0;
     const operator = criteria.operator || '>='; const statKey = criteria.stat;
     ...
     if (targetValue <= 0) {
        const meetsCriteria = operator === '>=' ? currentValue >= targetValue : operator === '==' ? currentValue == targetValue : false;
    ...
    switch (operator) {
         ...
         default: console.warn(`Unsupported operator: ${operator}`); meetsCriteria = false; progressPercent = 0; break;
    }
    progressPercent = Math.max(0, Math.min(100, progressPercent));
    return { progress: Math.floor(progressPercent), currentValue, targetValue, meetsCriteria };
}

// --- Display Achievements Section (with updated error handling) ---
async function displayAchievementsSection(competitiveStats, unlockedAchievementIds) {
    if (!achievementsSectionOuter || !achievementsListContainer) { return; }
    ...
    if (!currentAchievements) { // Attempt fetch ONLY if not cached
        ...
        if (!currentAchievements || Object.keys(currentAchievements).length === 0) { // Check if fetch failed or returned empty
            ...
            if (!allAchievements) allAchievements = {}; // Ensure global reflects state
            return; // Stop
        }
    }

    achievementsListContainer.innerHTML = ''; achievementsSectionOuter.style.display = 'block';
    const achievementIds = Object.keys(currentAchievements || {});
    if (achievementIds.length === 0) { achievementsListContainer.innerHTML = '<p class="list-message">No achievements defined yet.</p>'; return; }

    console.log(`Displaying ${achievementIds.length} achievements.`);
    achievementIds.forEach(achievementId => {
        const achievement = currentAchievements[achievementId];
        if (!achievement || !achievement.name || !achievement.criteria) { console.warn(`Skipping invalid achievement: ${achievementId}`); return; }
        ...
        const itemDiv = document.createElement('div'); itemDiv.classList.add('achievement-item');
        if (isUnlocked) itemDiv.classList.add('achievement-unlocked'); if (isDisplayCompleted) itemDiv.classList.add('achievement-completed');
        let rewardsHtml = '';
        if (achievement.rewards) { const parts = []; if (achievement.rewards.title) parts.push(`Title: <strong>${achievement.rewards.title}</strong>`); if (achievement.rewards.rank) parts.push(`Rank: <strong>${achievement.rewards.rank}</strong>`); if (parts.length > 0) { rewardsHtml = `<div class="achievement-rewards">Reward${parts.length > 1 ? 's': ''}: ${parts.join(', ')}</div>`; } }
        ...
        else { progressBarTitle = achievement.criteria.stat ? `${achievement.criteria.stat} Progress` : 'Progress'; }
        itemDiv.innerHTML = `<h4><span>${achievement.name}</span>${isDisplayCompleted ? '<span class="completion-icon" title="Completed!">✔</span>' : ''}</h4><p class="achievement-description">${achievement.description || 'No description.'}</p>${achievement.criteria.stat && achievement.criteria.value !== undefined ? `<div class="achievement-progress-container" title="${progressBarTitle}"><div class="achievement-progress-bar" style="width: ${progressInfo.progress}%;"><span>${progressText}</span></div></div>` : '<div style="height: 5px;"></div>'}${rewardsHtml}`;
...
async function checkAndGrantAchievements(userId, currentUserProfile, competitiveStats) {
    ...
    try {
        ...
        for (const achievementId in allAchievements) {
            if (unlockedIds.includes(achievementId)) continue; const achievement = allAchievements[achievementId]; if (!achievement?.criteria) continue;
            const progressInfo = calculateAchievementProgress(achievement, competitiveStats);
            if (progressInfo.meetsCriteria) {
                ...
                if (achievement.rewards) {
                    if (achievement.rewards.title && !profileToUpdate.availableTitles.includes(achievement.rewards.title)) { profileToUpdate.availableTitles.push(achievement.rewards.title); needsProfileUpdate = true; if (profileToUpdate.equippedTitle === "") { profileToUpdate.equippedTitle = achievement.rewards.title; } }
                    if (achievement.rewards.rank) { const currentIdx = rankOrder.indexOf(profileToUpdate.currentRank); const newIdx = rankOrder.indexOf(achievement.rewards.rank); const bestIdx = bestRankReward ? rankOrder.indexOf(bestRankReward) : -1; if (newIdx > Math.max(currentIdx, bestIdx)) { bestRankReward = achievement.rewards.rank; needsProfileUpdate = true; } }
                    if (achievement.rewards.bannerUrl && !profileToUpdate.bannerUrl) { profileToUpdate.bannerUrl = achievement.rewards.bannerUrl; needsProfileUpdate = true; }
                }
            }
        }
        if (bestRankReward) { const currentIdx = rankOrder.indexOf(profileToUpdate.currentRank); const bestIdx = rankOrder.indexOf(bestRankReward); if (bestIdx > currentIdx) { profileToUpdate.currentRank = bestRankReward; } }
        if (needsProfileUpdate || needsUserAchievementsUpdate) {
            ...
            if (needsProfileUpdate) {
                ...
                if (Object.keys(profileUpdateData).length > 0) { batch.update(userProfileRef, profileUpdateData); } else { needsProfileUpdate = false; }
            }
            if (needsProfileUpdate || needsUserAchievementsUpdate) { await batch.commit(); return profileToUpdate; } else { return null; }
        } else { return null; }
    } catch (error) { console.error(`Error checking/granting achievements for ${userId}:`, error); return null; }
}
...
function displayUserBadges(profileData) {
     ...
     for (const badgeType in badgeConfig) {
        ...
        if (config.emails.includes(userEmail)) {
            const badgeSpan = document.createElement('span'); badgeSpan.classList.add('profile-badge', config.className);
            badgeSpan.setAttribute('title', config.title); profileBadgesContainer.appendChild(badgeSpan);
        }
     }
}
...
function updateProfileTitlesAndRank(profileData, allowInteraction) {
    ...
    titleDisplay.classList.remove('selectable-title', 'no-title-placeholder'); titleDisplay.removeEventListener('click', handleTitleClick); closeTitleSelector();
    if (profileData && typeof profileData === 'object') {
        const rank = profileData.currentRank || 'Unranked'; const equippedTitle = profileData.equippedTitle || ''; const availableTitles = profileData.availableTitles || [];
        rankDisplay.textContent = rank; rankDisplay.className = `profile-rank-display rank-${rank.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}`;
        if (allowInteraction && availableTitles.length > 0) {
            titleDisplay.classList.add('selectable-title'); titleDisplay.addEventListener('click', handleTitleClick);
            if (equippedTitle) { titleDisplay.textContent = equippedTitle; titleDisplay.classList.remove('no-title-placeholder'); titleDisplay.style.display = 'inline-block'; }
            else { titleDisplay.textContent = '[Choose Title]'; titleDisplay.classList.add('no-title-placeholder'); titleDisplay.style.display = 'inline-block'; }
        } else {
            if (equippedTitle) { titleDisplay.textContent = equippedTitle; titleDisplay.classList.remove('no-title-placeholder'); titleDisplay.style.display = 'inline-block'; }
...
function handleTitleClick(event) {
    ...
    if (!isOwnProfile || !viewingUserProfileData.profile || !(viewingUserProfileData.profile.availableTitles?.length > 0)) { return; }
    if (isTitleSelectorOpen) { closeTitleSelector(); } else { openTitleSelector(); }
}
...
function openTitleSelector() {
    ...
    const availableTitles = viewingUserProfileData.profile.availableTitles; const currentEquippedTitle = viewingUserProfileData.profile.equippedTitle || '';
    if (!titleSelectorElement || !profileIdentifiersDiv.contains(titleSelectorElement)) { titleSelectorElement = document.createElement('div'); titleSelectorElement.className = 'title-selector'; profileIdentifiersDiv.appendChild(titleSelectorElement); }
    titleSelectorElement.innerHTML = '';
    if (currentEquippedTitle) { const opt = document.createElement('button'); opt.className = 'title-option title-option-unequip'; opt.dataset.title = ""; opt.type = 'button'; opt.textContent = '[Remove Title]'; opt.onclick = handleTitleOptionClick; titleSelectorElement.appendChild(opt); }
    availableTitles.forEach(title => { const opt = document.createElement('button'); opt.className = 'title-option'; opt.dataset.title = title; opt.type = 'button'; opt.textContent = title; if (title === currentEquippedTitle) { opt.classList.add('currently-equipped'); opt.disabled = true; } opt.onclick = handleTitleOptionClick; titleSelectorElement.appendChild(opt); });
...
function closeTitleSelector() {
    ...
    titleSelectorElement.style.display = 'none'; isTitleSelectorOpen = false; document.removeEventListener('click', handleClickOutsideTitleSelector, { capture: true });
}

function handleClickOutsideTitleSelector(event) {
    if (!isTitleSelectorOpen) return; const isInside = titleSelectorElement?.contains(event.target); const isOnTitle = titleDisplay?.contains(event.target);
    if (!isInside && !isOnTitle) { closeTitleSelector(); } else { setTimeout(() => { document.addEventListener('click', handleClickOutsideTitleSelector, { capture: true, once: true }); }, 0); }
}

async function handleTitleOptionClick(event) {
    event.stopPropagation(); const buttonElement = event.currentTarget; const selectedTitle = buttonElement.dataset.title; const currentUserId = loggedInUser?.uid;
    if (!currentUserId || !viewingUserProfileData.profile || viewingUserProfileData.profile.id !== currentUserId) { closeTitleSelector(); return; }
    const currentlyEquippedTitle = viewingUserProfileData.profile.equippedTitle || '';
    if (selectedTitle === currentlyEquippedTitle) { closeTitleSelector(); return; }
    closeTitleSelector(); titleDisplay.classList.remove('selectable-title', 'no-title-placeholder'); titleDisplay.removeEventListener('click', handleTitleClick); titleDisplay.textContent = "Updating...";
    try {
        const userProfileRef = db.collection('users').doc(currentUserId); await userProfileRef.update({ equippedTitle: selectedTitle });
        viewingUserProfileData.profile.equippedTitle = selectedTitle; if(isOwnProfile && viewerProfileData) viewerProfileData.equippedTitle = selectedTitle;
        saveCombinedDataToCache(currentUserId, viewingUserProfileData); updateProfileTitlesAndRank(viewingUserProfileData.profile, true);
    } catch (error) { console.error("Error updating title:", error); alert("Failed to update title."); if(viewingUserProfileData.profile) { viewingUserProfileData.profile.equippedTitle = currentlyEquippedTitle; updateProfileTitlesAndRank(viewingUserProfileData.profile, true); } else { updateProfileTitlesAndRank(null, false); } }
}
...
function setupProfilePicEditing() {
    if (!isOwnProfile || !editProfilePicIcon || !profilePicInput) { if(editProfilePicIcon) editProfilePicIcon.style.display = 'none'; return; }
    editProfilePicIcon.style.display = 'flex'; editProfilePicIcon.onclick = null; profilePicInput.onchange = null;
    editProfilePicIcon.onclick = () => { croppingFor = 'pfp'; profilePicInput.click(); };
    profilePicInput.onchange = (event) => { handleFileSelect(event); };
}

function setupBannerEditing() {
    if (!isOwnProfile || !editBannerIcon || !bannerInput) { if(editBannerIcon) editBannerIcon.style.display = 'none'; return; }
    editBannerIcon.style.display = 'flex'; editBannerIcon.onclick = null; bannerInput.onchange = null;
    editBannerIcon.onclick = () => { croppingFor = 'banner'; bannerInput.click(); };
...
function handleFileSelect(event) {
    const file = event.target?.files?.[0]; if (!file) { event.target.value = null; return; }
     if (!file.type.startsWith('image/')) { alert('Invalid image file.'); event.target.value = null; return; }
     const maxSizeMB = 8; if (file.size > maxSizeMB * 1024 * 1024) { alert(`File exceeds ${maxSizeMB}MB limit.`); event.target.value = null; return; }
    const reader = new FileReader();
    reader.onload = (e) => { if (e.target?.result) { modalImage.src = e.target.result; openEditModal(); } else { alert("Error reading file."); } };
    reader.onerror = (err) => { alert("Error reading file."); }; reader.readAsDataURL(file); event.target.value = null;
}
...
function openEditModal() {
    ...
    editModal.style.display = 'flex'; modalImage.style.opacity = 0;
    modalApplyBtn.disabled = false; modalSpinner.style.display = 'none'; const txt = Array.from(modalApplyBtn.childNodes).find(n => n.nodeType === Node.TEXT_NODE); if (txt) txt.textContent = 'Apply ';
    if (cropper) { try { cropper.destroy(); } catch(e) {} cropper = null; }
    const cropperOptions = { viewMode: 1, dragMode: 'move', background: false, autoCropArea: 0.9, responsive: true, modal: true, guides: true, center: true, highlight: false, cropBoxMovable: true, cropBoxResizable: true, toggleDragModeOnDblclick: false, ready: () => { modalImage.style.opacity = 1; console.log("Cropper ready for:", croppingFor); } };
    if (croppingFor === 'pfp') { cropperOptions.aspectRatio = 1 / 1; } else if (croppingFor === 'banner') { cropperOptions.aspectRatio = 3 / 1; } else { closeEditModal(); return; }
    setTimeout(() => { try { cropper = new Cropper(modalImage, cropperOptions); } catch (e) { console.error("Cropper init error:", e); alert("Could not init editor."); closeEditModal(); } }, 50);
    modalCloseBtn.onclick = closeEditModal; modalCancelBtn.onclick = closeEditModal; modalApplyBtn.onclick = handleApplyCrop; editModal.onclick = (e) => { if (e.target === editModal) closeEditModal(); };
}

function closeEditModal() {
    if (!editModal) return; if (cropper) { try { cropper.destroy(); } catch (e) {} cropper = null; }
    editModal.style.display = 'none'; modalImage.src = ''; modalImage.removeAttribute('src'); croppingFor = null;
    modalApplyBtn.disabled = false; modalSpinner.style.display = 'none'; const txt = Array.from(modalApplyBtn.childNodes).find(n => n.nodeType === Node.TEXT_NODE); if (txt) txt.textContent = 'Apply ';
    modalCloseBtn.onclick = null; modalCancelBtn.onclick = null; modalApplyBtn.onclick = null; editModal.onclick = null;
}

async function handleApplyCrop() {
    if (!cropper || !loggedInUser || !croppingFor) { alert("Cannot apply crop."); return; } if (modalApplyBtn.disabled) return;
    modalApplyBtn.disabled = true; modalSpinner.style.display = 'inline-block'; const txt = Array.from(modalApplyBtn.childNodes).find(n => n.nodeType === Node.TEXT_NODE); if (txt) txt.textContent = 'Applying ';
    try {
        const outputWidth = (croppingFor === 'banner') ? 1500 : 512; const outputHeight = (croppingFor === 'banner') ? 500 : 512;
        const canvas = cropper.getCroppedCanvas({ width: outputWidth, height: outputHeight, imageSmoothingEnabled: true, imageSmoothingQuality: 'high' }); if (!canvas) throw new Error("Failed to get canvas.");
        const blob = await new Promise((res, rej) => { canvas.toBlob((b) => { if (b) res(b); else rej(new Error("Blob conversion failed.")); }, 'image/jpeg', 0.90); });
        const imageUrl = await uploadToCloudinary(blob, croppingFor); await saveImageUrlToFirestore(loggedInUser.uid, imageUrl, croppingFor);
        if (croppingFor === 'pfp') { profileImage.src = `${imageUrl}?t=${Date.now()}`; profileImage.style.display = 'block'; profileInitials.style.display = 'none'; } else if (croppingFor === 'banner') { displayBanner(imageUrl); }
        if (viewingUserProfileData?.profile?.id === loggedInUser.uid) { if (croppingFor === 'pfp') { viewingUserProfileData.profile.profilePictureUrl = imageUrl; } else if (croppingFor === 'banner') { viewingUserProfileData.profile.bannerUrl = imageUrl; } if (viewerProfileData?.id === loggedInUser.uid) { if (croppingFor === 'pfp') viewerProfileData.profilePictureUrl = imageUrl; else if (croppingFor === 'banner') viewerProfileData.bannerUrl = imageUrl; } saveCombinedDataToCache(loggedInUser.uid, viewingUserProfileData); }
        closeEditModal();
    } catch (error) { console.error(`Error during ${croppingFor} save:`, error); alert(`Failed to update ${croppingFor}: ${error.message || 'Unknown error.'}`); modalApplyBtn.disabled = false; modalSpinner.style.display = 'none'; if (txt) txt.textContent = 'Apply '; }
}

async function uploadToCloudinary(blob, type = 'image') {
    if (!CLOUDINARY_CLOUD_NAME || !CLOUDINARY_UPLOAD_PRESET) { throw new Error("Cloudinary config missing."); } const url = `https://api.cloudinary.com/v1_1/${CLOUDINARY_CLOUD_NAME}/image/upload`; const formData = new FormData(); const filename = `${type}_${loggedInUser?.uid || 'anon'}_${Date.now()}.jpg`; formData.append('file', blob, filename); formData.append('upload_preset', CLOUDINARY_UPLOAD_PRESET); if (type === 'banner') { formData.append('folder', 'user_banners'); } else if (type === 'pfp') { formData.append('folder', 'user_pfps'); } console.log(`Uploading ${type} to Cloudinary...`);
...
async function saveImageUrlToFirestore(userId, imageUrl, type) {
    if (!userId || !imageUrl || !type) { throw new Error("Missing data for saving image URL."); } const userDocRef = db.collection("users").doc(userId); const fieldToUpdate = (type === 'banner') ? 'bannerUrl' : 'profilePictureUrl';
    try { await userDocRef.update({ [fieldToUpdate]: imageUrl }); console.log(`Successfully updated ${fieldToUpdate} for ${userId}`); } catch (e) { throw new Error(`Database error saving ${type} link.`); }
}
...
auth.onAuthStateChanged(async (user) => {
    ...
    loggedInUser = user; const targetUid = profileUidFromUrl || loggedInUser?.uid;
    viewerProfileData = null; viewingUserProfileData = {}; miniProfileCache = {}; isOwnProfile = loggedInUser && targetUid === loggedInUser.uid;
    if (targetUid) {
        console.log(`Targeting profile UID: ${targetUid}`);
        if (loadingIndicator) loadingIndicator.style.display = 'flex'; if (notLoggedInMsg) notLoggedInMsg.style.display = 'none'; if (profileArea) profileArea.style.display = 'none';
        try { await loadCombinedUserData(targetUid); console.log("Initial data load complete.");
            if (viewingUserProfileData.profile) { if (isOwnProfile) { setupProfilePicEditing(); setupBannerEditing(); } else { if(editProfilePicIcon) editProfilePicIcon.style.display = 'none'; if(editBannerIcon) editBannerIcon.style.display = 'none'; } profileLogoutBtn.style.display = isOwnProfile ? 'inline-block' : 'none'; }
        ...
        console.log('No user logged in and no profile UID.'); if (loadingIndicator) loadingIndicator.style.display = 'none'; if (profileArea) profileArea.style.display = 'none';
        if (notLoggedInMsg) { notLoggedInMsg.style.display = 'flex'; notLoggedInMsg.innerHTML = 'Please <a href="index.html#login-section" style="color: var(--primary-orange); margin: 0 5px;">log in</a> or provide a user ID.'; }
        if (adminTag) adminTag.style.display = 'none'; if (profileBadgesContainer) profileBadgesContainer.innerHTML = ''; if (profileLogoutBtn) profileLogoutBtn.style.display = 'none'; if (editProfilePicIcon) editProfilePicIcon.style.display = 'none'; if (editBannerIcon) editBannerIcon.style.display = 'none';
        updateProfileTitlesAndRank(null, false); displayBanner(null); if (competitiveStatsDisplay) competitiveStatsDisplay.innerHTML = ''; if(competitiveStatsSectionWrapper) competitiveStatsSectionWrapper.style.display = 'none'; if(poxelStatsSectionWrapper) poxelStatsSectionWrapper.style.display = 'none';
        closeTitleSelector(); closeEditModal(); clearFriendshipControls(); resetFriendsSection(); if (achievementsSectionOuter) achievementsSectionOuter.style.display = 'none';
...
profileLogoutBtn.addEventListener('click', () => {
    const userId = loggedInUser?.uid; console.log(`Logout clicked by: ${userId || 'N/A'}`);
    ...
    if (achievementsSectionOuter) achievementsSectionOuter.style.display = 'none'; if (competitiveStatsSectionWrapper) competitiveStatsSectionWrapper.style.display = 'none'; if (poxelStatsSectionWrapper) poxelStatsSectionWrapper.style.display = 'none';
    if (editProfilePicIcon) { editProfilePicIcon.onclick = null; editProfilePicIcon.style.display = 'none'; } if (profilePicInput) profilePicInput.onchange = null;
    if (editBannerIcon) { editBannerIcon.onclick = null; editBannerIcon.style.display = 'none'; } if (bannerInput) bannerInput.onchange = null;
    auth.signOut().then(() => {
        console.log('User signed out.'); if (userId) { localStorage.removeItem(`poxelProfileCombinedData_${userId}`); }
        loggedInUser = null; viewerProfileData = null; viewingUserProfileData = {}; miniProfileCache = {}; isOwnProfile = false; window.location.href = 'index.html';
...
function loadCombinedDataFromCache(viewedUserId) {
    ...
    try { const cachedDataString = localStorage.getItem(cacheKey); if (!cachedDataString) return false; const cachedData = JSON.parse(cachedDataString);
        if (cachedData?.profile?.id === viewedUserId) {
            cachedData.profile.friends = cachedData.profile.friends || {}; cachedData.profile.leaderboardStats = cachedData.profile.leaderboardStats || {}; cachedData.profile.availableTitles = cachedData.profile.availableTitles || []; cachedData.profile.equippedTitle = cachedData.profile.equippedTitle !== undefined ? cachedData.profile.equippedTitle : ""; cachedData.profile.currentRank = cachedData.profile.currentRank || "Unranked"; cachedData.profile.bannerUrl = cachedData.profile.bannerUrl || null; cachedData.stats = cachedData.stats || null;
...
function saveCombinedDataToCache(viewedUserId, combinedData) {
     if (!viewedUserId || !combinedData?.profile?.id || viewedUserId !== combinedData.profile.id) { return; } const cacheKey = `poxelProfileCombinedData_${viewedUserId}`;
    ...
    catch(error) { console.error(`Error saving cache for ${viewedUserId}:`, error); if (error.name === 'QuotaExceededError' || error.message?.toLowerCase().includes('quota')) { console.warn('LocalStorage quota exceeded.'); } }
}
